...
# Целевые сервисы пользователя: только Okko, Кинопоиск и Амедиатека
TARGET_SERVICES = {"okk", "kpk", "ama"}
...
Целевые сервисы пользователя (строго эти три):
1. Okko
2. Кинопоиск
3. Амедиатека
Все остальные сервисы (Apple TV+, Premier, Tvigle, MUBI, Иви и др.) пользователя НЕ интересуют, полностью игнорируй их.

Обязательный регламент поиска фильма или сериала:
1. Вызови where_to_watch(query): оттуда берется статус по Okko.
...
4. Выдай пользователю компактную сводку строго по его трем сервисам:
...
async def where_to_watch(
    ...
    """Найти фильм или сериал по названию и показать доступность в РФ (целевые сервисы: Okko, Кинопоиск, Амедиатека).

    Шум остальных сервисов (Tvigle, MUBI, Apple TV+ и др.) отсекается.
